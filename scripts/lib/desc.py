...
def render_layer(ℓ):
    annotation = (
        '[{:.1%} ✓ {:.1%} ×]'.format(ℓ['p_cor_ts'], ℓ['p_inc_ts'])
        if len(ℓ['sinks']) == 0 else '')
    sink_text = ''.join(
        '\n↳ ' + render_layer(s).replace(
            '\n', '\n| ' if i < len(ℓ['sinks']) - 1 else '\n  ')
        for i, s in enumerate(ℓ['sinks']))
    return '%s %s%s' % (ℓ['type'], annotation, sink_text)
...
def render_net_desc(desc, name='Network'):
    return (
        ...
        name, render_layer(desc['root']).replace('\n', '\n⋮   '),
        desc['acc_ts'], desc['mxe_ts'], desc['moc_ts'])
